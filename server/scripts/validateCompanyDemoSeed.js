...
check(
  isDevelopmentSeedEnabled,
  "Development seed data is not enabled",
);
...
const companyWorkspaceMembers =
  store.workspaceMembers.filter(
    (membership) =>
      companyWorkspaceIds.has(
        membership.workspaceId,
      ),
  );

const companyProjectMembers =
  store.projectMembers.filter(
    (membership) =>
      companyProjectIds.has(
...
check(
  ...
  `Expected 7 company users; ` +
    `received ${companyUsers.length}`,
);

check(
  companyWorkspaces.length === 6,
  `Expected 6 company Workspaces; ` +
    `received ${companyWorkspaces.length}`,
);
...
check(
  ...
  `Expected 18 company Projects; ` +
    `received ${companyProjects.length}`,
);
...
check(
  ...
  `Expected 90 company Tasks; ` +
    `received ${companyTasks.length}`,
);
...
Object.entries(collections).forEach(
  ([collectionName, items]) => {
    ...
    check(
      ...
      `Duplicate IDs in ${collectionName}: ` +
        `${duplicateIds.join(", ")}`,
    );
  },
);
...
check(
  ...
  "Company representative was not found",
);

companyWorkspaces.forEach(
  (workspace) => {
    const memberships =
      companyWorkspaceMembers.filter(
        (membership) =>
          membership.workspaceId ===
          workspace.id,
      );
    ...
    check(
      workspace.ownerId ===
        representative?.id,
      `${workspace.name} is not owned by ` +
        "the company representative",
    );

    check(
      memberships.length === 2,
      `${workspace.name} should have 2 members`,
    );
    ...
    check(
      ...
      `${workspace.name} should have ` +
        "one department Administrator",
    );

    check(
      projectCount === 3,
      `${workspace.name} should have ` +
        `3 Projects; received ${projectCount}`,
    );
  },
);

companyProjects.forEach(
  (project) => {
    const memberships =
      companyProjectMembers.filter(
        (membership) =>
          membership.projectId ===
          project.id,
      );
    ...
    check(
      ...
      `${project.name} references an ` +
        "invalid Workspace",
    );

    check(
      userIds.has(project.ownerId),
      `${project.name} has an invalid Owner`,
    );

    check(
      project.ownerId ===
        representative?.id,
      `${project.name} is not owned by ` +
        "the company representative",
    );
...
companyTasks.forEach((task) => {
  ...
  check(
    validStatusIds.has(task.status),
    `${task.title} has an invalid status`,
  );

  check(
    task.version === 1,
    `${task.title} should begin at version 1`,
  );
...
check(
  ...
  `Expected 33 Done Tasks; ` +
    `received ${statusCounts.done ?? 0}`,
);

check(
  statusCounts.doing === 23,
  `Expected 23 Doing Tasks; ` +
    `received ${statusCounts.doing ?? 0}`,
);

check(
  statusCounts.todo === 34,
  `Expected 34 To Do Tasks; ` +
    `received ${statusCounts.todo ?? 0}`,
);
...
if (failures.length > 0) {
  console.error(
    "Company demo seed validation failed:",
  );
  ...
  console.log(
    "Company demo seed validation passed.",
  );
  ...
  console.log({
    ...
    workspaceMemberships:
      companyWorkspaceMembers.length,
    projects: companyProjects.length,
    projectMemberships:
      companyProjectMembers.length,
